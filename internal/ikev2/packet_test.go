...
func TestPacketBytes(t *testing.T) {
	packet := &Packet{
		...
		Version:      0x20,
		ExchangeType: exchange.IKE_SA_INIT,
		Flags:        0x08, // initator
		MessageID:    0,
...
func makePacket() []byte {
	...
	return packet.AppendTo(nil)
}

func TestServer(t *testing.T) {
	serverAddr, err := net.ResolveUDPAddr("udp", "localhost:500")
	require.NoError(t, err)
	...
	for {
		...
		if len(packetData) == 1 && packetData[0] == 0xff {
			log.Printf("received keep-alive packet")
			continue
		}
